...
def _get_cycle_time(resolved_statuses, ticket) -> float:
    resolution_date = _get_resolution_date(resolved_statuses, ticket)
    creation_date = _get_creation_date(ticket)
...
def _calculate_average_cycle_time(resolved_statuses, debug_enabled, tickets) -> float:
    if tickets is None or len(tickets) == 0:
        raise Warning('No tickets found')
    total_cycle_time = 0
    for ticket in tickets:
        cycle_time = _get_cycle_time(resolved_statuses, ticket)
...
def _calculate_percentile_cycle_time(resolved_statuses: list[str], tickets, percentile) -> float:
    cycle_times = []
    for ticket in tickets:
        cycle_time = _get_cycle_time(resolved_statuses, ticket)
...
def show_project_cycletimes(config: Config, jira_client: JiraClient, percentiles: bool = False) -> None:
    ...
    for project in config.projects:
        ...
        print_info(f'\tAverage cycle time: {_calculate_average_cycle_time(config.resolved_statuses, config.debug_enabled, tickets):.2f} days')
        if percentiles:
            resolved_statuses = config.resolved_statuses
            print_info(
                f'\t50th percentile cycle time: {_calculate_percentile_cycle_time(resolved_statuses, tickets, 0.50):.2f} days')
            print_info(
                f'\t75th percentile cycle time: {_calculate_percentile_cycle_time(resolved_statuses, tickets, 0.75):.2f} days')
            print_info(
                f'\t85th percentile cycle time: {_calculate_percentile_cycle_time(resolved_statuses, tickets, 0.85):.2f} days')
        print_debug('')
